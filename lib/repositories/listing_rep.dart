...
//myzoo_2023@asia
  ...
  editProfileRep({String? userId,
  String? user_name, String? user_email,String? phone,String? user_nid,String? user_dob, String? user_address ,}) async {
    var response;
    APIManager _manager = APIManager();


      response = await _manager.postAPICall(ApiUrl.updateProfile, {
        "user_id": Get.find<AuthService>().currentUser.value.user!.userId.toString(),
        "user_name": user_name,
  ...
  String? booking_id,
    ...
    final response = await _manager
        .postAPICall("https://new.jayga.io/api/payment",
        {
  ...
  Future makeBooking({
    ...
    APIManager _manager = APIManager();
    final response =
        await _manager.postAPICall(ApiUrl.addBooking, {
      "user_id": userId,
      "lister_id": userId,
  ...
  Future makeReview({String? stars, String? listerId, String? listingId,String? review}) async {
    APIManager _manager = APIManager();
    final response = await _manager
        .postAPICall(ApiUrl.addReview, {
      "user_id": Get.find<AuthService>().currentUser.value.user!.userId.toString(),
      "lister_id": Get.find<AuthService>().currentUser.value.user!.userId.toString(),
      "listing_id": listingId,
      "stars": stars!,
      "review": review!,
    });
  ...
  Future paymentStatus({String? booking_id, String? listerId, String? listingId,String? pay_amount,}) async {
    APIManager _manager = APIManager();
    final response = await _manager
        .postAPICall(ApiUrl.paymentSTatusChanged, {
      "user_id": Get.find<AuthService>().currentUser.value.user!.userId.toString(),
      "lister_id": listerId,
  ...
  Future<GetListerProfileListingModel> getListerProfileListing(
    ...
    final response = await _manager.get("${ApiUrl.getListerProfileListing}$lister_id");
  ...
  Future changeListingActiveStatus(
    ...
    APIManager _manager = APIManager();
    print("api is ${ApiUrl.changeListingActiveStatus}");
    final response = await _manager.postAPICall(ApiUrl.changeListingActiveStatus, {
      "listing_id": listingID,
  ...
  Future editListingRep(
   ...
   String userData = Get.find<AuthService>().currentUser.value.user!.userId.toString();
